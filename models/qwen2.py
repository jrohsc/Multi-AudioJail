...
class Qwen2Assistant(VoiceAssistant):
    def __init__(self):
        self.processor = AutoProcessor.from_pretrained("Qwen/Qwen2-Audio-7B-Instruct", cache_dir='/work/pi_ahoumansadr_umass_edu/jroh/jroh_umass_edu/.cache/huggingface/hub')
        self.model = Qwen2AudioForConditionalGeneration.from_pretrained("Qwen/Qwen2-Audio-7B-Instruct",
                                                                        device_map="cuda",
                                                                        cache_dir='/work/pi_ahoumansadr_umass_edu/jroh/jroh_umass_edu/.cache/huggingface/hub', torch_dtype='auto')
    ...
    def generate_audio_defense(
        ...
        generate_ids = self.model.generate(**inputs, max_length=8024)
        generate_ids = generate_ids[:, inputs.input_ids.size(1):]
